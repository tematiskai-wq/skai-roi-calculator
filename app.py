...
if "Базовый Мониторинг" in selected_modules:
    with st.sidebar.expander("Модуль: Базовый Мониторинг", expanded=True):
        ...
        # Затраты перехода (Switching cost) при наличии старого GPS: аудит, замена SIM, переконфигурация
        if is_gps_installed:
        ...
        # Взвешенный расчет базового потенциала экономии топлива по категориям ТС
        ...
        # Модуль не претендует на предотвращение аварий (acc_weight = 0)
...
# Совместная вероятность предотвращения аварий (только активные системы безопасности)
...
for m_name, m_data in modules_raw.items():
    ...
    # Страховой дисконт закреплен эксклюзивно за Видеоаналитикой
...
with st.expander("Методология и математический аппарат расчетов", expanded=False):
    ...
    * **Базовый Мониторинг:** Направлен на ликвидацию прямых операционных потерь (устранение нецелевого пробега, контроль регламента ТО и соблюдения скоростных лимитов для снижения штрафов). Не учитывает предотвращение ДТП.
    * **Видеоаналитика и Безопасное вождение:** Являются активными системами предупреждения критических инцидентов. Их совокупный эффект предотвращения ДТП рассчитывается по формуле независимых вероятностей, исключающей задвоение эффекта:
    $$E_{дтп} = 1 - (1 - E_{видео}) \times (1 - E_{скоринг})$$
    * **Страховой дисконт:** Скидка при пролонгации парковых договоров КАСКО и ОСАГО признается страховыми андеррайтерами и начисляется исключительно при оснащении парка комплексами компьютерного зрения (Видеоаналитика).
    ...
    * **Стартовые инвестиции (Капитальные вложения):** Сумма единовременных затрат на закупку оборудования, монтаж или аудит/перевод парка:  
...
with st.expander("Обоснование показателей эффективности и источники данных", expanded=False):
    ...
    * **Сокращение пробега и ГСМ (%):**
        * **Источник:** Эксплуатационная статистика внедрений SKAI. Для магистральных перевозок потенциал исключения съездов с маршрута составляет 5–7%, для спецтехники — 8–10%, для городского развоза — 12–15%. При наличии ранее установленного GPS эффект составляет 3–6% за счет нормализации одометра и диспетчеризации.

...
with st.expander("Обоснование показателей эффективности и источники данных", expanded=False):
    ...
    * **Сокращение пробега и ГСМ (%):**
        * **Источник:** Методология внедрения SKAI при максимальном сценарии использования (подключение CAN-шины и аналитического отчета «Поездки водителей»). Для неоснащенных парков экономия достигает 15–22% за счет полного исключения нецелевых рейсов, приписок и оптимизации холостого хода. Для парков с уже установленным сторонним GPS максимальный эффект составляет 7–10% за счет исключения завышений одометра и перевода контроля на платформу SKAI.
        * **Влияние на расчет:** Мультипликатор применяется напрямую к ежемесячной статье «Затраты на ГСМ (Топливо)».
    * **Сокращение износа и ТО (%):**
        * **Источник:** Методология внедрения SKAI. Ликвидация перепробегов и снижения моточасов холостого хода отодвигает межсервисные интервалы. Максимальный эффект составляет 4–6% для новых парков и 3–4% для парков с существующим GPS.

...
presets = {
    ...
    "📦 Легкий коммерческий транспорт (LCV / Газель / Медпреды)": {
...
default_qtys = {
    ...
    "📦 Легкий коммерческий транспорт (LCV / Газель / Медпреды)": 15
}
...
with st.sidebar.expander("🔍 Потери бэк-офиса, простои и лизинг", expanded=True):
    ...
    emp_salary = st.number_input("Затраты на 1 сотрудника в месяц (ФОТ+налоги)", value=100000, step=5000)
    emp_revenue = st.number_input("Месячный доход от 1 сотрудника/медпреда", value=1200000, step=50000)
    
    st.markdown("**🚨 Издержки при инцидентах**")
    downtime_days = st.number_input("Средний простой ТС после ДТП (дней)", value=14, step=1)
    manager_hourly_rate = st.number_input("Стоимость 1 часа работы менеджера", value=500, step=50)
...
# Полная стоимость 1 ДТП с учетом скрытых факторов
# Прямой ущерб + ФОТ за простой + Упущенная выручка за простой + Время бэк-офиса
...
available_modules = ["Видеоаналитика", "Базовый Мониторинг", "Безопасное вождение", "Контроль топлива"]
selected_modules = []
...
def get_weighted_value(field):
    return sum(cp[field] * cp["qty"] for cp in custom_fleet_params.values()) / total_fleet_size

total_capex = 0
total_opex_monthly = 0

# Словари для разделения эффектов
direct_savings_dict = {}
tco_savings_dict = {}
...
if "Базовый Мониторинг" in selected_modules:
    with st.sidebar.expander("📍 Модуль: Базовый Мониторинг", expanded=True):
        ...
        eff_acc = st.slider("Сокращение ДТП (контроль геозон и простоев) (%)", 0, 100, int(get_weighted_value("base_eff_acc")), step=5) / 100

        # Прямые эффекты
        b_direct_saving = (total_fuel_before * eff_fuel) + (total_maint_before * eff_to) + ((total_direct_accident_damage_before / 12) * eff_acc)
        # Косвенные TCO эффекты
...
if "Видеоаналитика" in selected_modules:
    with st.sidebar.expander("👁️ Модуль: Видеоаналитика", expanded=True):
        ...
        # Видеоаналитика бьет строго по аварийности (и прямой, и TCO-составляющей)
...
calc_mode = st.radio(
    ...
    ["🔹 Только Прямой Экономический Эффект (Классический)", "🔥 Полный TCO Расчет (С учетом скрытых потерь, лизинга и персонала)"],
    horizontal=True
)

# Вычисление финальных результатов в зависимости от режима
...
tco_table_data = [
    ...
    ["Прямой ущерб от аварий / франшизы", fmt(total_direct_accident_damage_before / 12), fmt(sum(direct_savings_dict.values()) - (direct_savings_dict.get("Контроль топлива", 0) + total_fuel_before * eff_fuel if "Базовый Мониторинг" in selected_modules else 0)), "Прямой эффект"],
...
# Корректировка отображения таблицы
...
st.markdown(f"""
> **💡 Как новые факторы влияют на окупаемость (Выводы для защиты кейса):**
> * **Разница в моделях:** Переключитесь между *Прямым* и *Полным TCO* расчетом вверху страницы. Вы увидите, что учет упущенной выручки медпредов/водителей и стоимости простоев в ремонте снижает реальный срок окупаемости системы почти в **1.5 - 2 раза**.
> * **Сила Базового мониторинга:** Оцифровка времени менеджеров на обработку штрафов и снижение штрафов лизинговой компании при возврате авто доказывают, что даже простейший трекер окупает себя без масштабных вложений.
""")